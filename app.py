...
with st.sidebar:
    ...
    if selected_ind == "🔍 全台股手動輸入":
        ...
        ticker_symbol, ticker_name = f"{code}.TW", f"自選標 ({code})"
    else:
...
if sig:
    ...
    if not ledger_df.empty:
        ...
        st.download_button("📥 下載完整歷史交易紀錄 (CSV)", data=csv, file_name=f"{ticker_name}_TSMOM_Report.csv")
        
        items_per_page = 10
        total_pages = max(1, (len(ledger_df) - 1) // items_per_page + 1)
        page = st.number_input(f"📄 分頁瀏覽 (共 {total_pages} 頁)", min_value=1, max_value=total_pages, value=1)
        
        for _, row in ledger_df.iloc[(page-1)*items_per_page : page*items_per_page].iterrows():
