    ...
    try:
        ...
        for i in range(120, len(df)):
            ...
            if curr_w > 0 and prev_w == 0:
                ...
                # 防呆斷行：將超長日誌拆解為字串相加
                msg_buy = "**【TSMOM 動能進場】** 多重時間尺度 (20/60/120日) 趨勢共振分數達 " + str(round(row['Confidence'], 2)) + "，正式觸發做多訊號。\n\n"
                msg_buy += "**【風險與資金控管】** 目前 YZ 年化波動率為 " + str(round(row['YZ_Vol'] * 100, 1)) + "%。系統依據風險平價模型，自動調配 " + str(round(curr_w * 100, 1)) + "% 的資金水位。\n\n"
                msg_buy += "**【成交明細與點位】** 於 " + str(round(buy_price, 1)) + " 元進場建倉，共投入約 " + str(int(invested)) + " TWD。下檔防守支撐看 " + str(round(row['Sup'], 1)) + "。"
                ...
                # 防呆斷行：將超長日誌拆解為字串相加
                msg_sell = "**【風控平倉保護】** 趨勢信心分數轉弱 (目前分數降至: " + str(round(row['Confidence'], 2)) + ")，或波動率大幅擴張，觸發系統強制保護機制。\n\n"
                msg_sell += "**【動能衰減狀態】** TSMOM 動能無法延續，系統判定此波段多頭暫告一段落，空手觀望以保護本金。\n\n"
                msg_sell += "**【損益結算報告】** 於 " + str(round(row['Close'], 1)) + " 元執行平倉。本筆波段實質盈虧金額為 **" + str(int(profit)) + " TWD**。"
